...
async function requestSensorPermission() {
    ...
    try {
        ...
        console.log('📱 デバイス判定:', { isIOS, isAndroid, isDesktop });
            ...
            try {
                ...
                if (typeof DeviceMotionEvent.requestPermission === 'function') {
                    try {
                        ...
                        console.warn('⚠️ DeviceMotion許可要求エラー:', motionError);
                ...
                if (orientationPermission === 'granted') {
                    ...
                    await startSensorListening();
                    return;
                } else {
                    console.log('❌ iOS センサー許可拒否:', orientationPermission);
                    throw new Error(`センサーアクセスが拒否されました: ${orientationPermission}`);
                }
            } catch (permError) {
                console.error('❌ iOS許可要求エラー:', permError);
                throw permError;
            }
        } 
        // Android、旧iOS、デスクトップ
        else {
            console.log('🤖 非iOS環境 - 直接センサー開始');
            
            // センサーAPIが利用可能かチェック
            if ('DeviceOrientationEvent' in window) {
                console.log('✅ DeviceOrientationEvent が利用可能');
                permissionGranted = true;
                closePermissionModal();
                await startSensorListening();
                ...
                setTimeout(() => {
                    if (compassHeading === 0 && tiltX === 0 && tiltY === 0) {
                        console.warn('⚠️ センサー値が取得できていません - より積極的な初期化を試行');
                        // センサーイベントを強制的に試行
                        forceInitializeSensors();
                    }
                }, 1000);
            } else {
                console.warn('⚠️ DeviceOrientationEvent が利用できません');
                throw new Error('このデバイスではセンサーAPIが利用できません');
            }
        }
...
// センサーの強制初期化
function forceInitializeSensors() {
    console.log('🔧 センサーの強制初期化を試行中...');
    ...
    // 少し待ってから再登録
    setTimeout(() => {
        console.log('🔄 センサーイベントリスナーを再登録');
        window.addEventListener('deviceorientation', handleOrientation, { passive: true });
        window.addEventListener('devicemotion', handleMotion, { passive: true });
        
        // デバイスを動かすよう促す
        if ('navigator' in window && 'vibrate' in navigator) {
            navigator.vibrate([100, 100, 100]); // 振動でユーザーに知らせる
        }
        
        // アラートでユーザーに促す
        setTimeout(() => {
            if (compassHeading === 0 && tiltX === 0 && tiltY === 0) {
                alert('デバイスを少し動かしてセンサーを起動してください。');
            }
        }, 2000);
    }, 500);
}
...
function showSensorStatus() {
    ...
    statusDiv.innerHTML = `
        ✅ センサーアクセス許可完了<br>
        <small>デバイスを動かしてゲームを開始してください</small>
    `;
    ...
    setTimeout(() => {
        if (statusDiv && statusDiv.parentNode) {
            statusDiv.style.animation = 'slideUp 0.5s ease-in';
            setTimeout(() => {
                statusDiv.remove();
                style.remove();
            }, 500);
...
function startSensorListening() {
    ...
    try {
        ...
        const maxSensorChecks = 5;
        
        const sensorCheckInterval = setInterval(() => {
            sensorCheckCount++;
            console.log(`📊 センサーチェック ${sensorCheckCount}/${maxSensorChecks} - 値:`, { compassHeading, tiltX, tiltY });
            
            // センサー値が取得できている場合
            if (compassHeading !== 0 || tiltX !== 0 || tiltY !== 0) {
                console.log('✅ センサーが正常に動作しています');
            ...
            // 最大チェック回数に達した場合
            if (sensorCheckCount >= maxSensorChecks) {
                console.warn('⚠️ センサーが反応しません - 手動テストモードを開始');
                clearInterval(sensorCheckInterval);
                ...
                if (confirm(statusMessage)) {
                    ...
                    console.log('🧪 テストモードを開始しました');
                    startDemoMode();
                } else {
                    // センサー許可を再試行
                    console.log('🔄 センサー許可を再試行します');
                    requestSensorPermission();
                }
            }
        }, 2000); // 2秒間隔でチェック
...
function handleOrientation(event) {
    ...
    // イベントが無効でないかチェック
    if (!event) {
        console.warn('⚠️ 無効なorientationイベント');
        return;
    }
    ...
    if (heading === null || heading === undefined || isNaN(heading)) {
        ...
        if (Math.random() < 0.01) { // 1%の確率
            console.warn('⚠️ コンパス値を取得できません - フォールバック値を使用');
        }
        // 以前の値を保持するか、デフォルト値を使用
        heading = compassHeading || 0;
    }
    
    // 傾きセンサー値の取得（nullチェック付き）
    let newTiltX = event.beta;
    let newTiltY = event.gamma;
    
    // NaNや無効な値のチェック
    if (newTiltX === null || newTiltX === undefined || isNaN(newTiltX)) {
        newTiltX = tiltX || 0; // 以前の値を保持
    }
    if (newTiltY === null || newTiltY === undefined || isNaN(newTiltY)) {
        newTiltY = tiltY || 0; // 以前の値を保持
    }
    
    // 0-360の範囲に正規化
    let newCompassHeading = Math.round((parseFloat(heading) + 360) % 360);
    newTiltX = Math.round(parseFloat(newTiltX));
    newTiltY = Math.round(parseFloat(newTiltY));
    
    // 極端な値のクランプ（センサーノイズ対策）
    newTiltX = Math.max(-90, Math.min(90, newTiltX));
    newTiltY = Math.max(-90, Math.min(90, newTiltY));
    
    // センサーヒストリーに追加
    addToSensorHistory(newCompassHeading, newTiltX, newTiltY);
    
    // 値が変化した場合のみ更新（不要な処理を避ける）
    const hasChanged = 
        Math.abs(compassHeading - newCompassHeading) > 1 ||
        Math.abs(tiltX - newTiltX) > 1 ||
        Math.abs(tiltY - newTiltY) > 1;
    ...
    if (hasChanged) {
        ...
        // 初回センサー値受信をマーク
        if (!window.firstSensorValueReceived) {
            console.log('✅ 初回センサー値受信:', { compassHeading, tiltX, tiltY });
            window.firstSensorValueReceived = true;
        }
    }
...
function updateSensorDisplaySmooth() {
    ...
    if (compassValueEl) {
        const compassValue = Math.round(smoothCompassHeading);
        compassValueEl.textContent = `${compassValue}°`;
        
        // センサーが動作していることを視覚的に示す
        if (window.firstSensorValueReceived) {
            compassValueEl.style.color = '#ffffff';
            compassValueEl.style.fontWeight = '400';
        } else {
            compassValueEl.style.color = '#666666';
            compassValueEl.style.fontWeight = '300';
        }
    }
    
    if (tiltXEl) {
        const tiltXValue = Math.round(smoothTiltX);
        tiltXEl.textContent = `${tiltXValue > 0 ? '+' : ''}${tiltXValue}°`;
        
        // 傾きに応じて色を変更
        if (Math.abs(tiltXValue) > 10) {
            tiltXEl.style.color = '#ffffff';
        } else {
            tiltXEl.style.color = '#999999';
        }
    }
    
    if (tiltYEl) {
        const tiltYValue = Math.round(smoothTiltY);
        tiltYEl.textContent = `${tiltYValue > 0 ? '+' : ''}${tiltYValue}°`;
        
        // 傾きに応じて色を変更
        if (Math.abs(tiltYValue) > 10) {
            tiltYEl.style.color = '#ffffff';
        } else {
            tiltYEl.style.color = '#999999';
        }
    }
}
...
function checkSensorPermissionStatus() {
    ...
    if (permissionModal) {
        ...
        // iOS以外でも常に許可が必要であることを明示
        if (!isIOS13Plus) {
            console.log('⚠️ 非iOS環境でもセンサー許可が必要です');
        }
    } else {
        console.error('❌ 許可モーダル要素が見つかりません');
        // フォールバック：アラートで許可を求める
        if (confirm('センサーアクセスが必要です。許可しますか？')) {
            requestSensorPermission();
        }
    }
}
